...
const titleCase = (s) => s.toLowerCase().replace(/(^|[\s'-])([a-z])/g, (_, sep, ch) => sep + ch.toUpperCase());
...
function renderReport(record) {
  ...
  const statusCopy = {
    rising: [
      "Rising",
      `More babies were named ${record.name} in the last five years than in the five before that — the name is gaining ground.`,
    ],
    ...
    endangered: [
      "Endangered",
      `${record.name} has fallen ${a.declineFromPeakPct}% from its peak and only ${fmt(a.latest)} babies received it in ${record.yM}.`,
    ],
    extinct: [
      "Extinct",
      `No babies have been named ${record.name} in ${record.yM - a.lastYear} years. It last appeared in ${a.lastYear}, when ${fmt(record.series[a.lastYear])} ${sexLabel} were given the name.`,
    ],
  }[a.status];
  ...
  return `
<article class="report" data-name="${record.name}" data-sex="${record.sex}" data-status="${a.status}">
  <h1>${record.name}</h1>
...
async function setupSearch(input, suggestions, submit, sexSelect) {
  ...
  const render = () => {
    ...
    suggestions.innerHTML = currentSuggestions
      .map(
        (s, i) =>
          `<div data-i="${i}" class="${i === activeIdx ? "active" : ""}"><span>${s.name}</span><span class="meta">${s.sex}${s.peakCount ? " · peak " + fmt(s.peakCount) : ""}</span></div>`,
      )
      .join("");
  ...
  input.addEventListener("keydown", (e) => {
    if (!suggestions.classList.contains("hidden")) {
      if (e.key === "ArrowDown") {
        ...
        pick(currentSuggestions[activeIdx]);
        e.preventDefault();
        return;
